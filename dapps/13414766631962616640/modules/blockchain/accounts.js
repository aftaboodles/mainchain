...
private.addAccount = function (account, scope) {
	...
	account.balance["BEL"] = account.balance["BEL"] || 0;
	account.u_balance["BEL"] = account.u_balance["BEL"] || 0;
...
Accounts.prototype.generateAddressByPublicKey = function (publicKey) {
	...
	var address = bignum.fromBuffer(temp).toString() + "BL";
	return address;
}
...
Accounts.prototype.mergeAccountAndGet = function (data, cb, scope) {
	console.log(data, '  merge Account Get')
	var address = data.address || null;
	if (address === null) {
		if (data.publicKey) {
			address = self.generateAddressByPublicKey(data.publicKey);
...
Accounts.prototype.undoMerging = function (data, cb, scope) {
	console.log(data, '  undo merge Account Get')
	var address = data.address || null;
	if (address === null) {
		if (data.publicKey) {
			address = self.generateAddressByPublicKey(data.publicKey);
