...
CREATE INDEX IF NOT EXISTS "mem_accounts_get_delegates" ON "mem_accounts" ("ranking" DESC, ENCODE("publicKey", 'hex') ASC) WHERE "isDelegate" = 1;

-- Add 'mem_accounts_block_id' index for 'blockId'
CREATE INDEX IF NOT EXISTS "mem_accounts_block_id" ON "mem_accounts" ("blockId");

/*-- Add 'mem_accounts_is_verify' index for 'status'
CREATE INDEX IF NOT EXISTS "mem_accounts_is_verify" ON "mem_accounts" ("status");

CREATE INDEX IF NOT EXISTS "mem_accounts_get_ac_status" ON "mem_accounts" ("vote" DESC, ENCODE("publicKey", 'hex') ASC) WHERE "isDelegate" = 1;
CREATE INDEX IF NOT EXISTS "mem_accounts_get_delegates" ON "mem_accounts" ("ranking" DESC, ENCODE("publicKey", 'hex') ASC) WHERE "isDelegate" = 1;
*/
