...
function Account (scope, cb) {
	...
	this.model = [
		...
		},
		{
			...
			conv: Array,
			expression: '(SELECT ARRAY_AGG("dependentId") FROM ' + this.table + '2ac_status WHERE "accountId" = a."address")'
		},
		{
			name: 'u_ac_status',
			type: 'Text',
			...
			conv: Array,
			expression: '(SELECT ARRAY_AGG("dependentId") FROM ' + this.table + '2u_ac_status WHERE "accountId" = a."address")'
		},
		{
			name: 'rankings',
			type: 'Text',
			...
			conv: Array,
			expression: '(SELECT ARRAY_AGG("dependentId") FROM ' + this.table + '2rankings WHERE "accountId" = a."address")'
		},
		{
			name: 'u_rankings',
			type: 'Text',
			...
			conv: Array,
			expression: '(SELECT ARRAY_AGG("dependentId") FROM ' + this.table + '2u_rankings WHERE "accountId" = a."address")'
		},
		{
...
Account.prototype.removeTables = function (cb) {
	...
	[this.table,
		...
		'mem_accounts2ac_status',
		'mem_accounts2u_ac_status',,
		'mem_accounts2rankings',
		'mem_accounts2u_rankings',
...
Account.prototype.merge = function (address, diff, cb) {
				...
				if (Object.prototype.toString.call(trueValue[0]) === '[object Object]') {
					...
					for (i = 0; i < trueValue.length; i++) {
						...
						if (math === '-') {
							...
							val = trueValue[i].slice(1);
							//console.log('Insede Delegate 4 ', value);
							insert[value] = insert[value] || [];
							insert[value].push(val);
							if (value === 'delegates') {
								//console.log('Insede Delegate 2');
								round.push({
									query: 'INSERT INTO mem_round ("address", "amount", "delegate", "blockId", "round") SELECT ${address}, (balance)::bigint, ${delegate}, ${blockId}, ${round} FROM mem_accounts WHERE address = ${address};',
									values: {
										address: address,
							...
							val = trueValue[i];
							//console.log('Insede Delegate 5 ', value);
							insert[value] = insert[value] || [];
							insert[value].push(val);
							if (value === 'delegates') {
								//console.log('Insede Delegate 3');
								round.push({
									query: 'INSERT INTO mem_round ("address", "amount", "delegate", "blockId", "round") SELECT ${address}, (balance)::bigint, ${delegate}, ${blockId}, ${round} FROM mem_accounts WHERE address = ${address};',
									values: {
										address: address,
	...
	var sqles = [];
	//console.log(insert, "   this.editable ::::::::::::::::::::::::::::::    ");
