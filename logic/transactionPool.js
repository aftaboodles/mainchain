...
TransactionPool.prototype.addBundledTransaction = function (transaction) {
	//console.log('Transaction in queue :: 5');
...
TransactionPool.prototype.addQueuedTransaction = function (transaction) {
	//console.log('Transaction in queue :: 7', transaction);
...
TransactionPool.prototype.addMultisignatureTransaction = function (transaction) {
	//console.log('Transaction in queue :: 6');
...
TransactionPool.prototype.receiveTransactions = function (transactions, broadcast, cb) {
	//console.log('TransactionPool.prototype.receiveTransactions :::: ');
	console.log(transactions);
	async.eachSeries(transactions, function (transaction, cb) {
		self.processUnconfirmedTransaction(transaction, broadcast, cb);
	}, function (err) {
		//console.log('ReceiveTransactions Error :: ', err);
...
TransactionPool.prototype.processUnconfirmedTransaction = function (transaction, broadcast, cb) {
	//console.log('ProcessUnconfirmedTransaction :: ')
...
TransactionPool.prototype.queueTransaction = function (transaction, cb) {
	...
	if (transaction.bundled) {
		...
		if (self.countMultisignature() >= config.transactions.maxTxsPerQueue) {
			...
			//console.log('Transaction in queue :: 3');
		...
		if (self.countQueued() >= config.transactions.maxTxsPerQueue) {
			...
			//console.log('Transaction in queue :: 4');
...
/**
 * [checkVrificationOnKYC description]
 * @param  {[type]}   sender [description]
 * @param  {[type]}   trs    [description]
 * @param  {Function} cb     [description]
 * @return {[type]}          [description]
 */
__private.checkVrificationOnKYCWithoutAPI = function(sender, trs, cb) {
	library.logger.info('******************** Using custom field to verify the KYC ************************')
	var recipientId = trs.recipientId;
	if (trs.type === transactionTypes.ACCOUNT) {
		var payload = sender.address;
		httpCall.call('GET', '/api/v1/accounts/status?walletAddressArray='+ payload, null, function(error, result){
			if(!error && result){
				if((result.data[sender.address+''] && trs.asset.ac_status.status != 1) || (!result.data[sender.address+''] && trs.asset.ac_status.status != 0)) {
		...
		cb(sender.address + ' wallet is not verified.');
	} else {
		modules.accounts.getAccount({address : recipientId}, function (err, row){
			if(!row || row.status != 1){
				cb(recipientId + ' wallet is not verified.');
			} else {
				cb();
			}
		})
...
__private.checkVrificationOnKYCThroughAPI = function(sender, trs, cb) {
	cb();
		...
		httpCall.call('GET', '/api/v1/accounts/status?walletAddressArray='+payload, null, function(error, result){
			if(!error && result && result.data){
				...
				addresses.forEach(function(address){
					if(!result.data[address+''] && address != '6232988412937045830BL' && sender.address != '6232988412937045830BL'){
...
__private.processVerifyTransaction = function(transaction, broadcast, cb) {
	...
	modules.accounts.setAccountAndGet({
		...
		/**
		 * Used to check account status (Account verified or not)
		 **/
		 var kycCheck = {};
		 kycCheck.api = (library.logic.walletVerificationAPI.enable) ? __private.checkVrificationOnKYCThroughAPI : __private.checkVrificationOnKYCWithoutAPI;
			...
			if (err) {
				...
				async.waterfall([
					...
					function processTransaction(sender, requester, waterCb) {
						//console.log('processTransaction 1111');
					...
					function verifyTransaction(sender, waterCb) {
						//console.log('verifyTransaction 1111');
						library.logic.transaction.verify(transaction, sender, function(err) {
							//console.log('Verify >>>>>>  ', err, sender);
							if (err) {
					...
					//console.log('Final Result :::: ', err, sender);
					if (!err) {
