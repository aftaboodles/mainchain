...
set_network() {
  if [ "$(grep "7337a324ef27e1e234d1e9018cacff7d4f299a09c2df9be460543b8f7ef652f1" $BELRIUM_CONFIG )" ];then
    NETWORK="main"
  elif [ "$(grep "cba57b868c8571599ad594c6607a77cad60cf0372ecde803004d87e679117c12" $BELRIUM_CONFIG )" ];then
    NETWORK="test"
  else
    NETWORK="unknown"
  fi
...
BELRIUM_CONFIG="config.json"
DB_NAME="$(grep "database" $BELRIUM_CONFIG | cut -f 4 -d '"' | head -1)"
DB_UNAME="$(grep "user" $BELRIUM_CONFIG | cut -f 4 -d '"' | head -1)"
DB_PASSWD="$(grep "password" $BELRIUM_CONFIG | cut -f 4 -d '"' | head -1)"
DB_SNAPSHOT="blockchain.db.gz"
NETWORK=""
set_network
BLOCKCHAIN_URL="https://downloads.belrium.org/snapshot/$NETWORK"
...
ntp_checks() {
    ...
    if [[ ! -f "/proc/user_beancounters" ]]; then
      if ! sudo pgrep -x "ntpd" > /dev/null; then
        ...
        if ! sudo pgrep -x "ntpd" > /dev/null; then
          echo -e "BELRIUM requires NTP running. Please check /etc/ntp.conf and correct any issues. Exiting."
          exit 1
        echo -e "done.\n"
        fi # if sudo pgrep
...
install_webui() {
    ...
    echo -n "Installing Belrium WebUi... "
    git clone -b development https://pankaj005@github.com/oodlestechnologies/BelerumUI.git &>> $logfile || { echo -n "Could not clone git wallet source. Exiting." && exit 1; }
    ...
    if [[ -d "BelerumUI" ]]; then
        mv BelerumUI/wallet-ui public
    else
        echo "Could not find installation directory for BELRIUM web wallet. Install the web wallet manually."
        exit 1;
    fi
...
update_manager() {
    ...
    echo -n "Updating Belrium Manager ... "
    # checkout belrium_manager here
    echo "done."
...
update_client() {
    ...
    echo -n "Updating Belrium client ... "
...
update_wallet() {
    ...
    echo -n "Updating Belrium wallet ... "
...
start_belrium() {
    echo -n "Starting Belrium... "
    forever_exists=$(whereis forever | awk {'print $2'})
    if [[ ! -z $forever_exists ]]; then
        $forever_exists start -o $root_path/logs/belrium_console.log -e $root_path/logs/belrium_err.log app.js -c "$BELRIUM_CONFIG" &>> $logfile || \
        { echo -e "\nCould not start Belrium." && exit 1; }
    fi
  ...
  start_snapshot() {
    echo -n "Starting Belrium (snapshot mode) ... "
    forever_exists=$(whereis forever | awk {'print $2'})
    if [[ ! -z $forever_exists ]]; then
        $forever_exists start -o $root_path/logs/snapshot_console.log -e $root_path/logs/snapshot_err.log app.js -c "$BELRIUM_CONFIG" -s highest &>> $logfile || \
        { echo -e "\nCould not start Belrium." && exit 1; }
    fi
...
install_ipfs() {
  ...
  ipfs_exists=$(whereis ipfs | awk {'print $2'})
    if [[ ! -z $ipfs_exists ]]; then
      echo -e "IPFS is already installed. Remove it first with ./belrium_manager.bash remove_ipfs"
      exit 1;
    fi
  ...
  PORT="$(jq .port $BELRIUM_CONFIG)"
  SPORT="$(jq .ssl.options.port $BELRIUM_CONFIG)"
...
case $1 in
    "install")
      ...
      echo ""
      echo ""
      echo "BELRIUM successfully installed"
    ...
    "status")
      if running; then
        echo "√ BELRIUM is running."
        show_blockHeight
      else
        echo "X BELRIUM is NOT running."
      fi
    ;;
    "start")
      parse_option $@
...
*)
    ...
    echo 'Usage: ./belrium_installer.bash install'
    exit 1
;;
esac
exit 0;