...
/*	if (!Object.keys(__private.keypairs).length) {
	...
	//console.log(':::::::::::::, !__private.loaded - ',!__private.loaded, ', modules.loader.syncing() - ', modules.loader.syncing(), ', !modules.rounds.loaded() - ', !modules.rounds.loaded(), ', modules.rounds.ticking()- ', modules.rounds.ticking())
...
__private.checkDelegatesRanking = function (publicKey, ranking, state, cb) {
	if (!Array.isArray(ranking)) {
		return setImmediate(cb, 'Ranking must be an array');
	}

	console.log('Ranking ::::::::::::::::::::::::::::::::;   ',ranking, state);
	...
	modules.accounts.getAccount({publicKey: publicKey}, function (err, account) {
		...
		var delegates = (state === 'confirmed') ? account.rankings : account.u_rankings;
		var existing_ranking = Array.isArray(delegates) ? delegates.length : 0;
		var additions = 0, removals = 0;

		async.eachSeries(ranking, function (action, cb) {
			var math = action[0];
			...
			if (math === '+' && (delegates != null && delegates.indexOf(publicKey) !== -1)) {
				return setImmediate(cb, 'Failed to add rank, account has already voted for this delegate');
			}

			if (math === '-' && (delegates === null || delegates.indexOf(publicKey) === -1)) {
				return setImmediate(cb, 'Failed to remove rank, account has not voted for this delegate');
			}
			...
			var total_ranking = (existing_ranking + additions) - removals;

			if (total_ranking > constants.activeDelegates) {
				var exceeded = total_ranking - constants.activeDelegates;

				return setImmediate(cb, 'Maximum number of ' + constants.activeDelegates + ' ranking exceeded (' + exceeded + ' too many)');
			} else {
				return setImmediate(cb);
			}
		});
...
Delegates.prototype.getDelegates = function (query, cb) {
	...
	modules.accounts.getAccounts({
		isDelegate: 1,
		sort: { 'vote': -1, 'publicKey': 1 }
	}, ['username', 'address', 'publicKey', 'vote', 'missedblocks', 'producedblocks', 'ranking'], function (err, delegates) {
		if (err) {
			return setImmediate(cb, err);
		}
...
Delegates.prototype.checkConfirmedDelegatesRanking = function (publicKey, votes, cb) {
	return __private.checkDelegatesRanking(publicKey, votes, 'confirmed', cb);
};

Delegates.prototype.checkUnconfirmedDelegatesRanking = function (publicKey, votes, cb) {
	return __private.checkDelegatesRanking(publicKey, votes, 'unconfirmed', cb);
};
	...
	getDelegates: function (req, cb) {
		library.schema.validate(req.body, schema.getDelegates, function (err) {
			...
			modules.delegates.getDelegates(req.body, function (err, data) {
				...
				if (data.sortField) {
					// TODO: 'rate' property is deprecated and need to be removed after transitional period
					if (['approval', 'productivity', 'rate', 'rank', 'vote', 'ranking'].indexOf(data.sortField) > -1) {
						data.delegates = data.delegates.sort(compareNumber);
					} else if (['username', 'address', 'publicKey', 'ranking'].indexOf(data.sortField) > -1) {
