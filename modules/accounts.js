...
var Ranking = require('../logic/ranking.js');
var Acstatus = require('../logic/ac_status.js');
var scheduler = require('../logic/scheduler.js');
...
function Accounts (cb, scope) {
	...
	__private.assetTypes[transactionTypes.RANKING] = library.logic.transaction.attachAssetType(
		transactionTypes.RANKING, new Ranking()
	);
	__private.assetTypes[transactionTypes.ACCOUNT] = library.logic.transaction.attachAssetType(
		transactionTypes.ACCOUNT, new Acstatus()
	);
...
__private.openAccount = function (secret, cb) {
	...
	self.getAccount({publicKey: publicKey}, function (err, account) {
		...
		if (account) {
			...
			return setImmediate(cb, null, accountObject);
		}
	});
};

Accounts.prototype.openAccountForKYC = function(secret, cb){
	__private.openAccount(secret, cb);
}
// Public methods
Accounts.prototype.generateAddressByPublicKey = function (publicKey) {
	...
	var address = bignum.fromBuffer(temp).toString() + '' + constants.addressPostfix;
...
Accounts.prototype.onBind = function (scope) {
	...
	__private.assetTypes[transactionTypes.RANKING].bind({
		modules: modules, library: library
	});
	__private.assetTypes[transactionTypes.ACCOUNT].bind({
		modules: modules, library: library
	});
};
...
Accounts.prototype.shared = {
				...
				if (req.body.multisigAccountPublicKey && req.body.multisigAccountPublicKey !== keypair.publicKey.toString('hex')) {
					modules.accounts.getAccount({ publicKey: req.body.multisigAccountPublicKey }, function (err, account) {
						...
						modules.accounts.getAccount({ publicKey: keypair.publicKey }, function (err, requester) {
							...
							try {
								transaction = library.logic.transaction.create({
									type: transactionTypes.RANKING,
									ranking: req.body.rankings,
									sender: account,
									keypair: keypair,
									secondKeypair: secondKeypair,
									requester: keypair
	...
	generateAddress: function(req, cb) {
		if (!Array.isArray(req.body.payload)) {
			return setImmediate(cb, 'Missing required property: payload');
		}

		if(req.body.payload && Array.isArray(req.body.payload)){
			scheduler.wallet(modules.accounts, modules.transactions, req.body.payload, function(err, data) {
				return setImmediate(cb, err, {
					accounts: data
				});
			})
