...
function Transactions (cb, scope) {
	...
	__private.assetTypes[transactionTypes.SHARED_REWARDS] = library.logic.transaction.attachAssetType(
		transactionTypes.SHARED_REWARDS, new Transfer()
	);
	__private.assetTypes[transactionTypes.BASIC] = library.logic.transaction.attachAssetType(
		transactionTypes.BASIC, new Transfer()
	);
	__private.assetTypes[transactionTypes.CIVIL] = library.logic.transaction.attachAssetType(
		transactionTypes.CIVIL, new Transfer()
	);
	__private.assetTypes[transactionTypes.EDUCATION] = library.logic.transaction.attachAssetType(
		transactionTypes.EDUCATION, new Transfer()
	);
	__private.assetTypes[transactionTypes.FINANCIAL] = library.logic.transaction.attachAssetType(
		transactionTypes.FINANCIAL, new Transfer()
	);
...
__private.getRankingById = function (transaction, cb) {
	library.db.query(sql.getRankingById, {id: transaction.id}).then(function (rows) {
		if (!rows.length) {
			return setImmediate(cb, 'Transaction not found: ' + transaction.id);
		}

		var ranking = rows[0].ranking.split(',');
		var added = [];
		var deleted = [];

		for (var i = 0; i < ranking.length; i++) {
			if (ranking[i].substring(0, 1) === '+') {
				added.push (ranking[i].substring(1));
			} else if (ranking[i].substring(0, 1) === '-') {
				deleted.push (ranking[i].substring(1));
			}
		}

		transaction.ranking = {added: added, deleted: deleted};

		return setImmediate(cb, null, transaction);
	}).catch(function (err) {
		library.logger.error(err.stack);
		return setImmediate(cb, 'Transactions#getRankingById error');
	});
};
...
Transactions.prototype.initialCoin = function (params, cb) {
	self.shared.addTransactions(params, cb);
}
...
Transactions.prototype.shared = {
		...
		library.schema.validate(req.body, schema.getTransaction, function (err) {
			...
			__private.getById(req.body.id, function (err, transaction) {
				...
				if (transaction.type === 3) {
					...
					__private.getRankingById(transaction, function (err, transaction) {
						return setImmediate(cb, null, {transaction: transaction});
					});
				} else {
					return setImmediate(cb, null, {transaction: transaction});
				...
				modules.accounts.getAccount(query, function (err, recipient) {
					...
					if (req.body.multisigAccountPublicKey && req.body.multisigAccountPublicKey !== keypair.publicKey.toString('hex')) {
						...
						modules.accounts.setAccountAndGet({publicKey: keypair.publicKey.toString('hex')}, function (err, account) {
							...
							//console.log('Before transaction receive :: ', transaction);
					...
					if (req.body.multisigAccountPublicKey && req.body.multisigAccountPublicKey !== keypair.publicKey.toString('hex')) {
						...
						modules.accounts.getAccount({publicKey: req.body.multisigAccountPublicKey}, function (err, account) {
							...
							modules.accounts.getAccount({publicKey: keypair.publicKey}, function (err, requester) {
								...
								try {
									transaction = library.logic.transaction.create({
										type: transactionTypes.SHARED_REWARDS,
										amount: req.body.amount,
										sender: account,
										recipientId: recipientId,
										keypair: keypair,
	...
	verifyAccount: function (req, cb) {
		library.schema.validate(req.body, schema.verifyAccount, function (err) {
			if (err) {
				return setImmediate(cb, err[0].message);
			}
