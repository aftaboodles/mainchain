...
__private.senderRecipientAccount = {};
...
/*     These methods used in promise    */
/**
 * [applyUnconfirmedTransaction description]
 * @param  {[object]} transaction [description]
 * @param  {[Object]} m           [module]
 * @param  {[Object]} l           [library]
 * @return {[promise]}             [promise]
 */
/*function applyUnconfirmedTransaction(transaction, m, l) {
    return new Promise(function(res, rej) {
        __private.senderAndRecipient(transaction, function() {
            // DATABASE write
            m.accounts.setAccountAndGet({
                publicKey: transaction.senderPublicKey
            }, function(err, sender) {
                __private.senderRecipientAccount[transaction.id].sender = sender;
                // DATABASE: write
                m.transactions.applyUnconfirmed(transaction, sender, function(err) {
                    if (err) {
                        err = ['Failed to apply transaction:', transaction.id, '-', err].join(' ');
                        l.logger.error(err);
                        l.logger.error('Transaction', transaction);
                        res(null)
                    } else {
                        res(transaction);
                    }
...
function undoUnconfirmedTransaction(l, trs, sr) {
    return new Promises(function(resolve, reject) {
        l.logic.transaction.undoUnconfirmed(trs, sr[trs.id].sender, function(err, data) {
            resolve();
        });
    })
}*/

function applyConfirmedTransaction(transaction, block, sender) {
...
__private.applyBlock = function (block, broadcast, cb, saveBlock) {
	...
	async.series({
		...
		applyUnconfirmed: function (seriesCb) {
			...
			async.each(block.transactions, function (transaction, eachSeriesCb) {
				// DATABASE write
				modules.accounts.setAccountAndGet({publicKey: transaction.senderPublicKey}, function (err, sender) {
					// DATABASE: write
		
					__private.senderAndRecipient(transaction, function (){
						var sender = __private.senderRecipientAccount[transaction.id].sender; 
				...
				if (err) {
					...
					async.eachSeries(block.transactions, function (transaction, eachSeriesCb) {
						...
						console.log('::::::::::::::::::::::::::;   ApplyTime :::::', (new Date().getTime()-applyTime)/1000)
		...
		applyConfirmed: function (seriesCb) {
			...
			block.transactions.forEach(function (transaction){
				var sender = __private.senderRecipientAccount[transaction.id].sender;
				promises.push(applyConfirmedTransaction(transaction, block, sender));
			})
			if(promises.length > 0){
				Promise.all(promises).then(function (result){
					console.log(':::::::::::::   applyConfirmed :::::::::::::::::::: ', (new Date().getTime() - t)/1000);
					return setImmediate(seriesCb);
				}).catch (function (err){
					process.exit(0);
				})
		...
		saveBlock: function (seriesCb) {
			...
			if (saveBlock) {
				...
				__private.saveBlock(block, function (err) {
					console.log(':::::::::::::   saveBlock :::::::::::::::::::: ', (new Date().getTime() - t)/1000);
					if (err) {
		...
		console.log(':::::::::::::::::::: Total time taken for '+block.transactions.length+' transactions ::::::::::::::::::::::', (new Date().getTime() - time)/1000);
...
__private.checkTransaction = function (block, transaction, cb) {
	async.waterfall([
		...
		function (waterCb) {
			...
			// DATABASE: read block
			__private.senderAndRecipient(transaction, function (){
				library.logic.transaction.verify(transaction, __private.senderRecipientAccount[transaction.id].sender, waterCb);		
			})	
...
Blocks.prototype.loadBlocksFromPeer = function (peer, cb) {
	...
	console.log('__________________ Loading blocks _________________-');
	console.log('__________________ lastValidBlock _________________', lastValidBlock);
	peer = library.logic.peers.create(peer);
	library.logger.info('Loading blocks from: ' + peer.string);
	console.log('___________________ loading blocks from ___________________', peer, peer.string)
	function getFromPeer (seriesCb) {
		console.log('__________  getFromPeer blocks __________________')
		modules.transport.getFromPeer(peer, {
			method: 'GET',
			api: '/blocks?lastBlockId=' + lastValidBlock.id
		}, function (err, res) {
			console.log('__________  getFromPeer __________________', err, res)
	...
	function validateBlocks (blocks, seriesCb) {
		console.log('_______________validate blocks_________________')
	//	var report = library.schema.validate(blocks, schema.loadBlocksFromPeer);

		//if (!report) {
		//	console.log('___________ validateBlocks Error_____________')
		//	return setImmediate(seriesCb, 'Received invalid blocks data');
	//	} else {
			console.log('___________ validateBlocks Success _____________')
			return setImmediate(seriesCb, null, blocks);
	//	}
	}

	function processBlocks (blocks, seriesCb) {
		console.log('___________ processBlocks_____________')
		if (blocks.length === 0) {
			console.log('__________  Block length is zero _______________')
			return setImmediate(seriesCb);
		}
		async.eachSeries(__private.readDbRows(blocks), function (block, eachSeriesCb) {
			if (__private.cleanup) {
				console.log('__________  Process blocks error _______________')
				return setImmediate(eachSeriesCb);
			} else {
				return processBlock(block, eachSeriesCb);
			}
		}, function (err) {
			console.log('__________  Process blocks_error _______________')
			return setImmediate(seriesCb, err);
		});
	}
...
__private.senderAndRecipient = function(transaction, cb){
	if(!transaction || __private.senderRecipientAccount[transaction.id] 
		&& __private.senderRecipientAccount[transaction.id].sender){
		cb();
	} else {
		__private.senderRecipientAccount[transaction.id] = {}; 
		...
		var sender = modules.accounts.generateAddressByPublicKey(transaction.senderPublicKey);
	
		filter.address['$in'].push(sender);
		var requester;
		if(transaction.requesterPublicKey) {
			requester = modules.accounts.generateAddressByPublicKey(transaction.requesterPublicKey);
			filter.address['$in'].push(requester);
		}
		modules.accounts.getAccounts(filter, function (err, rows) {
			if (!err && rows) {
				rows.forEach(function (row) {
					if(row.address == sender){
						__private.senderRecipientAccount[transaction.id].sender = row;
					} else if (row.address == requester) {
						__private.senderRecipientAccount[transaction.id].requester = row;
					}
				})
			}
			cb();
...
Blocks.prototype.generateBlock = function (keypair, timestamp, cb) {
	...
	var ready = [];
	console.log(':::::::::::::::::::: Start generating block ::::::::::::::::::::::')
	async.eachSeries(transactions, function (transaction, cb) {
		__private.senderAndRecipient(transaction, function(){
			console.log(':::::::::::::::::::: After senderAndRecipient ::::::::::::::::::::::')
			var sender = __private.senderRecipientAccount[transaction.id].sender;
			if (!sender) {
		...
		console.log(':::::::::::::::::::: Start creating block Object ::::::::::::::::::::::')
		var block;
...
Blocks.prototype.processBlock = function (block, broadcast, cb, saveBlock) {
	...
	async.series({
		...
		if (err) {
			...
			console.log(':::::::::::::::::::: Before applying block ::::::::::::::::::::::',(new Date().getTime() - processTime)/1000);
